...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python imagenet_main.py $seed \
  ...
  -ec --cos_alpha 0.001 --learn_rate 0.018 --label_smoothing 0.1 -gft -eqz -oqz --q_bits 4 --q_mode 1 --copy_num 10 \
  -ek --version_t 14 --kd_size 101 --temp_dst 2. --w_dst 2. \
  --num_gpus 8 --train_epochs 90 --stop_threshold 0.7746 --batch_size 1024 \
  --version 34 --resnet_size 26 --epochs_between_evals 1 | tee "log/resnet_pruned_${seed}.log"
