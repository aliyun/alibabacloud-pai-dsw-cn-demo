...
################################################################################
def block_m(inputs, filters, training, strides, data_format):

  resnet_log_helper.log_begin_block(
      input_tensor=inputs, block_type=mlperf_log.BOTTLENECK_BLOCK)

  shortcut = conv2d_fixed_padding(
        inputs=inputs, filters=448, kernel_size=1, strides=strides,
        data_format=data_format)
  ...
  inputs = conv2d_fixed_padding(
      inputs=inputs, filters=448, kernel_size=1, strides=1,
      data_format=data_format)
  inputs = batch_norm(inputs, training, data_format)
...
def block_m0(inputs, filters, training, strides, data_format):
  ...
  inputs = conv2d_fixed_padding(
      inputs=inputs, filters=96, kernel_size=1, strides=1,
      data_format=data_format)
  inputs = batch_norm(inputs, training, data_format)
...
def block_m1(inputs, filters, training, strides, data_format):
  ...
  avg_pool = tf.layers.average_pooling2d(
      ...
      data_format=data_format)
  shortcut = conv2d_fixed_padding(
        inputs=avg_pool, filters=480, kernel_size=1, strides=1,
        data_format=data_format)
  resnet_log_helper.log_projection(input_tensor=inputs,
                                     output_tensor=shortcut)
  shortcut = batch_norm(inputs=shortcut, training=training,
  ...
  inputs = conv2d_fixed_padding(
      inputs=inputs, filters=480, kernel_size=1, strides=1,
      data_format=data_format)
  inputs = batch_norm(inputs, training, data_format)
...
def block_m2(inputs, filters, training, strides, data_format):
  ...
  inputs = conv2d_fixed_padding(
      inputs=inputs, filters=192, kernel_size=1, strides=1,
      data_format=data_format)
  inputs = batch_norm(inputs, training, data_format)
  ...
  inputs = conv2d_fixed_padding(
      inputs=inputs, filters=224, kernel_size=3, strides=strides,
      data_format=data_format)
  inputs = batch_norm(inputs, training, data_format)
