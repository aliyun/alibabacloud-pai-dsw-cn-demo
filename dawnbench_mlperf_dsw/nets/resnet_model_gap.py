...
def block_layer(inputs, filters, bottleneck, block_fn, blocks, strides,
  ...
  elif version == 24: # mlperf open
    projection_shortcut = projection_shortcut
  elif version == 34: # dawnbench
    projection_shortcut = projection_shortcut
...
class ModelGap(object):
  ...
  def __init__(self, resnet_size, bottleneck, num_classes,
               ...
               final_size, pickle_model='./gap_save/gap_pruned.pkl',
               version=DEFAULT_VERSION, data_format=None,
               dtype=DEFAULT_DTYPE, random_init=False, enable_at=False):
    ...
    if version not in (1, 2, 14, 24, 34):
      raise ValueError(
          'Resnet version should be 1 or 2. See README for citations.')

    self.bottleneck = bottleneck
    if bottleneck:
      if (version == 1) or (version == 14) or (version == 24) or (version == 34):
    ...
    pruned_save_pkl = pickle_model #'./gap_save/gap_pruned.pkl'
    if pruned_save_pkl.startswith("oss://"):
      with tf.gfile.GFile(pruned_save_pkl, "r") as f:
        gap_vars = pickle.load(f)
    else:
      with open(pruned_save_pkl, 'rb') as f:
        gap_vars = pickle.load(f)

    self.gf = gap_finetune.GapFinetune(gap_vars, random_init)
