...
def process_record_dataset(dataset, is_training, batch_size, shuffle_buffer,
                           ...
                           examples_per_epoch=None, dtype=tf.float32, mix_up=False):
...
def learning_rate_with_decay(
    batch_size, batch_denom, num_images, boundary_epochs, decay_rates, train_epochs=90,
    base_lr=0.1, enable_lars=False, enable_cos=False, cos_alpha=1e-4, warm_up=True):
  ...
  def learning_rate_fn(global_step):
    ...
    if warm_up:
        ...
        warmup_lr_init = initial_learning_rate
        warmup_lr = (
            warmup_lr_init * tf.cast(global_step, tf.float32) / tf.cast(
  ...
  def cosine_rate_fn(global_step):
    """ cosine annealing for lr adjustment """
    decay_steps = batches_per_epoch * train_epochs
    cosine_rate = tf.train.cosine_decay(
...
def batch_mix_up(features, onehot_labels, dtype, mx_mode=0):
  onehot_labels = math_ops.cast(onehot_labels, dtype)
  batch_size = tf.shape(onehot_labels)[0]
  indexs = tf.range(batch_size)
  if mx_mode==0: # random
    indexs = tf.random.shuffle(indexs)
  elif mx_mode==1: # reverse
    indexs = indexs[::-1]

  beta = tf.distributions.Beta(_MIXUP_ALPHA, _MIXUP_ALPHA)
  alpha = beta.sample(batch_size)
  image_alpha = tf.expand_dims(tf.expand_dims(tf.expand_dims(alpha, -1), -1), -1)
  label_alpha = tf.expand_dims(alpha, -1)

  x1, x2 = features, tf.gather(features, indexs, axis=0)
  y1, y2 = onehot_labels, tf.gather(onehot_labels, indexs, axis=0)

  features = image_alpha * x1 + (1 - image_alpha) * x2
  onehot_labels = label_alpha * y1 + (1 - label_alpha) * y2

  return features, onehot_labels


def resnet_model_fn(features, labels, mode, model_class, model_teach, resnet_size, random_init,
                    final_size, pickle_model, weight_decay, learning_rate_fn, momentum,
                    data_format, version, version_t, loss_scale, gap_train, gap_lambda, gap_ft,
                    loss_filter_fn=None, dtype=resnet_model.DEFAULT_DTYPE,
                    label_smoothing=0.0, enable_lars=False, enable_kd=False,
                    kd_size=50, temp_dst=4., w_dst=4., mix_up=False, mx_mode=0,
  ...
  # one_hot label or soft-prob label is used for label_smoothing
  if label_smoothing != 0.0:
    one_hot_labels = tf.one_hot(labels, 1001)
  else:
    one_hot_labels = labels

  # mixup is used, which can be considered as augmented large-scale dataset training trick
  if mix_up and (mode == tf.estimator.ModeKeys.TRAIN):
    features, one_hot_labels = batch_mix_up(features, one_hot_labels, dtype, mx_mode)
  else:
    features, one_hot_labels = features, one_hot_labels

  if gap_ft:
    # the pickle_model should be on-hands
    model = model_class(resnet_size, final_size, data_format,
                      pickle_model=pickle_model, version=version, dtype=dtype,
                      random_init=random_init, enable_at=enable_at)
  else:
    model = model_class(resnet_size, final_size, data_format, version=version,
                      dtype=dtype, enable_at=enable_at)

  logits, feat_s = model(features, mode == tf.estimator.ModeKeys.TRAIN)
  ...
  if label_smoothing != 0.0:
    # label_smoothing can be used after mixup
  ...
  # add KD, AT loss
  if enable_kd and (mode == tf.estimator.ModeKeys.TRAIN):
    model_t = model_teach(kd_size, data_format=data_format,
                          version=version_t, dtype=dtype, temp_dst=temp_dst,
                          w_dst=w_dst, enable_at=enable_at, w_at=w_at)
    logits_t, feat_t = model_t(features, False)
    logits_t = tf.stop_gradient(logits_t)
    cross_entropy += model_t.calc_loss_dst(logits, logits_t)
    if enable_at:
      cross_entropy += model_t.calc_loss_at(feat_t, feat_s)
  else:
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    if loss_scale != 1:
      ...
      if gap_train:
        # network slimming regularization
        l1_loss = gap_lambda * gap_train_prune.gap_get_l1_loss(tf.get_default_graph(), True)
        tf.summary.scalar('gap_l1_loss', l1_loss)
        loss = loss + l1_loss
...
def resnet_main(seed, flags, model_function, input_function, shape=None):
  ...
  classifier = tf.estimator.Estimator(
      ...
      params={
          ...
          'online_quantize': flags.online_quantize,
  ...
  for i in range(iter_train_epochs // flags.epochs_between_evals):
    ...
    classifier.train(input_fn=input_fn_train, hooks=train_hooks + extra_hooks,
...
class ResnetArgParser(argparse.ArgumentParser):
  ...
  def __init__(self, resnet_size_choices=None):
    ...
    self.add_argument(
        '--version', '-v', type=int, choices=[1, 2, 14, 24, 34],
        default=resnet_model.DEFAULT_VERSION,
        help='Version of ResNet. (1, 2, 14 or 24, 34) See README.md for details.'
    )

    self.add_argument(
        '--version_t', '-vt', type=int, choices=[1, 2, 14],
        default=resnet_model.DEFAULT_VERSION,
        help='Version of ResNet Teacher. (1, 2 or 14) See README.md for details.'
    )
    ...
    self.add_argument(
        '--final_size', '-fs', type=int, default=2048,
        help='[default: %(default)s] The final size for dense layer.',
        metavar='<FS>'
    )

    self.add_argument(
        "--pickle_model", "-pm", default="./gap_save/gap_pruned.pkl",
        help="[default: %(default)s] The location of the pruned model param file "
             "files.",
        metavar="<PM>",
    )

    self.add_argument(
       "--random_init", "-ri", action='store_true',
        help="[default: %(default)s] random_init: If True the gap fine-tune is from scratch."
    )

    self.add_argument(
       "--enable_kd", "-ek", action='store_true',
        help="[default: %(default)s] enable_kd: If True knowledge distillation is enabled."
    )

    self.add_argument(
        '--kd_size', '-ks', type=int, default=50,
        choices=resnet_size_choices,
        help='[default: %(default)s] The size of the ResNet model (teach) to use.',
        metavar='<KS>' if resnet_size_choices is None else None
    )

    self.add_argument(
       "--temp_dst", "-tdst", type=float, default=2.,
       help='[default: %(default)s] temp_dst: temperature for knowledge distillation.',
       metavar="<TDST>"
    )

    self.add_argument(
       "--w_dst", "-wdst", type=float, default=2.,
       help='[default: %(default)s] w_dst: balance factor for knowledge distillation.',
       metavar="<WDST>"
    )

    self.add_argument(
       "--mix_up", "-mu", action='store_true',
        help="[default: %(default)s] mix_up: If True mix_up is enabled."
    )

    self.add_argument(
        '--mx_mode', '-mmd', type=int, default=0,
        help='[default: %(default)s] the mode of mixup: randome or reverse.',
        metavar='<MMD>'
    )

    self.add_argument(
       "--enable_quantize", "-eqz", action='store_true',
        help="[default: %(default)s] enable_quantize: If True quantization-aware training is enabled."
    )

    self.add_argument(
       "--online_quantize", "-oqz", action='store_true',
        help="[default: %(default)s] enable_quantize: If True online quantization-aware training is enabled."
    )

    self.add_argument(
        '--q_bits', '-qbt', type=int, default=8,
        help='[default: %(default)s] quantization bits.',
        metavar='<QBT>'
    )

    self.add_argument(
        '--copy_num', '-cnum', type=int, default=10,
        help='[default: %(default)s] quantization copies.',
        metavar='<CNUM>'
    )

    self.add_argument(
        '--q_mode', '-qme', type=int, default=1,
        help='[default: %(default)s] quantization method (KL, MAX, Perc.).',
        metavar='<QME>'
    )

    self.add_argument(
       "--enable_at", "-eat", action='store_true',
        help="[default: %(default)s] enable_at: If True attention transfer is enabled."
    )

    self.add_argument(
       "--w_at", "-wat", type=float, default=8.,
       help='[default: %(default)s] w_at: balance factor for attention transfer.',
       metavar="<WDST>"
    )

    self.add_argument(
       "--oss_load", "-osl", action='store_true',
