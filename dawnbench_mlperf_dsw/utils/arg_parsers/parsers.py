...
class BaseParser(argparse.ArgumentParser):
  ...
  def __init__(self, add_help=False, data_dir=True, model_dir=True,
               ...
               stop_threshold=True, batch_size=True, warm_up=True,
               multi_gpu=False, num_gpu=True, hooks=True, gap_train=True, enable_cos=True,
               enable_lars=True, label_smoothing=True, weight_decay=True, fine_tune=True):
    super(BaseParser, self).__init__(add_help=add_help)

    if data_dir:
      self.add_argument(
          "--data_dir", "-dd", default="/data/ImageNet_TFRecorder",
    ...
    if model_dir:
      self.add_argument(
          "--model_dir", "-md", default="/models/resnet50_v1_mlperf",
    ...
    self.add_argument(
       "--learn_rate", "-lr", type=float, default=0.128,
    ...
    if enable_cos:
      self.add_argument(
         "--enable_cos", "-ec", action='store_true',
         help='[default: %(default)s] Enable cosine annealing.'
      )

      self.add_argument(
         "--cos_alpha", "-ca", type=float, default=1e-4,
         help='[default: %(default)s] alpha for cosine annealing',
         metavar="<CA>"
      )
    ...
    if warm_up:
      self.add_argument(
         "--warm_up", "-wu", action='store_true',
         help='[default: %(default)s] Enable warmup for training.'
      )
    ...
    if gap_train:
      self.add_argument(
         "--gap_train", "-gt", action='store_true',
          help="[default: %(default)s] gap_train: If True the gap sparsity constraint is applied."
      )

      self.add_argument(
         "--gap_lambda", "-gl", type=float, default=1e-4,
         help='[default: %(default)s] gap_lambda: Balance factor for gap training with l1 loss.',
         metavar="<GL>"
      )

      self.add_argument(
         "--gap_ft", "-gft", action='store_true',
          help="[default: %(default)s] gap_ft: If True the gap fine-tune for pruned model is applied."
      )

      self.add_argument(
          "--gap_start", "-gst", type=int, default=0,
          help="[default: %(default)s] which epoch the gap training starts from. ",
          metavar="<GST>"
      )
